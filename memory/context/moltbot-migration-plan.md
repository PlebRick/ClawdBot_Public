...
- ✅ GitHub org created Jan 4, 2026 — legitimate, same developer (steipete)
- ✅ `package.json` in the repo now says `"name": "moltbot"` with version `2026.1.26`
- ✅ Both `moltbot` and `clawdbot` are registered as bin commands in package.json
- ✅ Docs moved to `docs.molt.bot`
- ✅ Website at `molt.bot`
...
- ❌ **The latest real npm release is still `clawdbot@2026.1.24-3`** — this is what we're running
- ❌ **No `moltbot` release has been published with actual code yet**
...
2. Updated the source code's `package.json` to say `moltbot`
...
- **Website:** molt.bot
- **Docs:** docs.molt.bot
- **npm (current):** `clawdbot` (still the real package)
- **npm (future):** `moltbot` (placeholder only — DO NOT INSTALL)
- **GitHub:** moltbot/moltbot
- **Discord:** unchanged (discord.gg/clawd)
...
| Binary path | `~/.nvm/versions/node/v22.22.0/bin/clawdbot` |
...
| Config backups | `~/.clawdbot/clawdbot.json.bak` (×5) |
| Credentials | `~/.clawdbot/credentials/` (telegram-allowFrom, telegram-pairing, webchat-pairing) |
...
| Var | Where | Purpose |
|-----|-------|---------|
| `CLAWDBOT_NODE_OPTIONS_READY` | Process env | Internal flag |
| `CLAWDBOT_SYSTEMD_UNIT` | User service file | Service identification |
| `CLAWDBOT_SERVICE_MARKER` | User service file | Service detection |
...
- ✅ Migrate config file names if needed
- ✅ Normalize deprecated config keys
- ✅ Migrate on-disk state layouts (sessions, agent dirs)
- ✅ Check and repair service configs
...
moltbot --version    # should show real version
...
|-------|-----------|--------|------------|
| **npm placeholder trap** — installing `moltbot@0.1.0` breaks everything | HIGH (right now) | Total breakage | **DO NOT install moltbot from npm until real version is published** |
| **Config file rename confusion** — docs say `moltbot.json`, code may still expect `clawdbot.json` | Medium | Gateway won't start | Let `moltbot doctor` handle; check what version actually expects |
| **Two systemd services conflict** — port 18789 double-bind | Medium | Gateway crash | Disable old services before enabling new |
| **Custom system service not detected by doctor** | Certain | Doctor skips our service | Manual service update required |
| **Skills path change** — skills installed under `clawdbot` node_modules | Medium | Skills not found | Reinstall skills: `moltbot skills install` |
...
|------|-----------|----------|--------|
| **`clawdbot` npm package deprecated** | High | When `moltbot` npm is published | Update warnings, eventual removal |
| **Compatibility shim dropped** | Medium | 3-6 months after real moltbot npm | `clawdbot` command stops working |
| **docs.clawd.bot goes offline** | Medium | Already redirects to docs.molt.bot | Lose documentation |
| **New features only in `moltbot`** | Certain | Already happening in source | Miss features, bug fixes, security patches |
| **Config schema diverges** | Medium | 3-6 months | Old config keys break |
...
Check periodically:
```bash
npm view moltbot version   # Currently 0.1.0 (placeholder)
